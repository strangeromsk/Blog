...
public interface PostRepository extends JpaRepository<Post, Integer>, PagingAndSortingRepository<Post, Integer> {
    @Query(value = "SELECT * FROM posts WHERE is_active = 1 AND moderation_status = 'ACCEPTED'" +
            " AND timestamp < CURRENT_TIMESTAMP ORDER BY timestamp", nativeQuery = true)
    List<Post> findPostByDateAsc (Pageable pageable);
    @Query(value = "SELECT * FROM posts WHERE is_active = 1 AND moderation_status = 'ACCEPTED'" +
            " AND timestamp < CURRENT_TIMESTAMP ORDER BY timestamp DESC", nativeQuery = true)
    List<Post> findPostByDateDesc (Pageable pageable);
    @Query(value = "FROM Post WHERE isActive = 1 AND status = 'ACCEPTED'" +
            " AND timestamp < CURRENT_TIMESTAMP ORDER BY size(postComments) DESC")
    ...
    @Query(value = "SELECT posts.* FROM posts LEFT JOIN post_votes ON posts.id=post_votes.post_id AND post_votes.value = 1" +
            " WHERE posts.is_active = 1 AND posts.moderation_status = 'ACCEPTED' AND posts.timestamp < CURRENT_TIMESTAMP" +
            " GROUP BY posts.id ORDER BY COUNT(post_votes.value) DESC", nativeQuery = true)
    List<Post> findPostByLikeCount (Pageable pageable);
    @Query(value = "SELECT * FROM posts WHERE is_active = 1 AND moderation_status = 'ACCEPTED'" +
            " AND timestamp < CURRENT_TIMESTAMP AND (text LIKE %:query% OR title LIKE %:query%)", nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(*) FROM posts WHERE is_active = 1 AND moderation_status = 'ACCEPTED'" +
            " AND timestamp < CURRENT_TIMESTAMP", nativeQuery = true)
    Long countPost();
    ...
    @Query(value = "SELECT * FROM posts JOIN tag2post ON posts.id=tag2post.post_id JOIN tags ON tags.id=tag2post.tag_id " +
            "WHERE posts.is_active = 1 AND posts.moderation_status = 'ACCEPTED' AND posts.timestamp < CURRENT_TIMESTAMP AND tags.name = :tag", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM posts WHERE is_active = 1 AND moderation_status = 'ACCEPTED'" +
            " AND timestamp < CURRENT_TIMESTAMP AND id =:id", nativeQuery = true)
    ...
    @Query(value = "SELECT DATE_FORMAT(from_unixtime(`time`),'%Y-%m-%d') AS date, COUNT(*) FROM posts WHERE YEAR(from_unixtime(timestamp)) = :year AND is_active = 1" +
    ...
    @Query(value = "SELECT * FROM posts WHERE is_active = 0" +
            " AND timestamp < CURRENT_TIMESTAMP AND user_id = :id ORDER BY timestamp", nativeQuery = true)
